...
const Shop = () => {
  ...
  if (!filteredProductsQuery.isLoading && filteredProductsQuery.data) {
    // Filter based on price input if available
    const filteredProducts = filteredProductsQuery.data.filter((product) => {

      if (!priceFilter) return true; // No filter applied
      return (
        product.price.toString().includes(priceFilter) ||
        product.price === parseInt(priceFilter, 10)
      );
...
//   // Add "All Brands" option to uniqueBrands
  ...
  return (
    <>
       <div className="container mx-auto ml-[13%]">
         <div className="flex md:flex-row">
           <div className="bg-[#151515] p-3 mt-2 mb-2">
             <h2 className="h4 text-center py-2 bg-black rounded-full mb-2 ml-20">
               Filter by Categories
             </h2>

             <div className="p-5 w-[15rem] ml-[4rem]">
              {categories?.map((c) => (
