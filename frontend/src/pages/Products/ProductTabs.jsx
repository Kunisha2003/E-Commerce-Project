...
const ProductTabs = ({
    ...
    <div className="flex flex-col">
                    ...
                    {userInfo ? (
                        <form onSubmit={submitHandler}>
                            ...
                            <div className="my-2">
                              ...
                              <textarea id="comment" required value={comment} rows="3" onChange={e=>setComment(e.target.value)}
                                className="p-2 border rounded-lg xl:w-[40rem] text-black">
                        ...
                        <p>Please <Link to="/login">Sign In</Link>to write a review
                        </p>
                    )}
        ...
        <section>
          {activeTab ===2 && (
            <>
            <div>{product.reviews.length === 0 && <p>No Reviews</p>}</div><div>
              {product.reviews.map((review) => (
                <div
                key = {review._id}
                className="bg-[#1A1A1A] p-4 rounded-lg xl:ml-[2rem] sm:ml-[0rem] xl:w-[50rem] sm:w-[24rem]">
                  
                  <div className="flex justify-between">
                    <strong className="text-[#B0B0B0]">{review.name}</strong>
                    <p className="text-[#B0B0B0]">{review.created_at.substring(0,10)}
